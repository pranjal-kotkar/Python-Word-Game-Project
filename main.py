...
def savename():
    ...
    for rank, (username, score) in enumerate(all_scores, start=1):
...
def animate():
    ...
    if not stop_animation:
        word.after(100, animate)#pranjal
...
b1=Button(word,borderwidth=10,bg="pink",font=("Helvetica", 14),fg="magenta",activeforeground="green",activebackground="black",text=current_sw[0],command=lambda:entry_update(current_sw[0]))
...
b6=Button(word,borderwidth=10,bg="yellow",font=("Helvetica", 14),fg="magenta",activeforeground="green",activebackground="black",text=current_sw[5],command=lambda:button_click(current_sw[5]))
...
def reset():
        global word_count,chancees,guessed_words
        words_count=0
        chances=6
        guessed_words.clear()
